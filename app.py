...
@app.route("/delete/<int:pg_id>", methods = ['GET','POST'])
@auth.login_required
def delete(pg_id):
    pg_id = pg_id
    blog = Posts.query.get(pg_id)
...
def verify_password(user, passwd):
    ...
    print(admin)
    if admin:
        if check_password_hash(admin.password_hash, passwd):
            res = True
        else:
            res = False
    else:
...
def get_admin():
    ...
    vr = verify_password(user, passwd)
    if vr:
